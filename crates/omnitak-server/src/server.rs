...
impl TakServer {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        if self.config.tls_port > 0 {
            if let Some(tls_config) = &self.config.tls {
                ...
                let tls_acceptor = tls::load_tls_config(
                ...
                let handle = tokio::spawn(async move {
                    Self::accept_tls_loop(listener, tls_acceptor, router, router_tx, timeout_secs, max_clients).await
                });

                self.tls_handle = Some(handle);
            } else {
                warn!("TLS port configured but no TLS config provided, skipping TLS listener");
            }
        }
    ...
    async fn accept_tls_loop(
        ...
        loop {
            ...
            match listener.accept().await {
                Ok((stream, addr)) => {
                    info!("Accepted TLS connection from {}", addr);
                    ...
                    tokio::spawn(async move {
                        ...
                        match tls_acceptor.accept(stream).await {
                            Ok(tls_stream) => {
                                ...
                                // Create client handler (which assigns ID)
                                let client = Client::new(tls_stream, addr, mpsc::channel(100).1, timeout_secs);
                                let client_id = client.info().id;
                                ...
                                // Handle client
                                match client.handle(router_tx).await {
                                    Ok(_) => info!("[Client {}] Disconnected normally", client_id),
                                    Err(e) => error!("[Client {}] Disconnected with error: {}", client_id, e),
                                }
