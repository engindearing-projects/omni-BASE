...
struct DrawingPropertiesView: View {
    ...
    private func saveChanges() {
        ...
        } else if var route = drawingStore.routes.first(where: { $0.id == drawingID }) {
            route.name = editedName
            route.color = editedColor
            drawingStore.updateRoute(route)
        } else if var circle = drawingStore.circles.first(where: { $0.id == drawingID }) {
            circle.name = editedName
    ...
    private func deleteDrawing() {
        if let marker = drawingStore.markers.first(where: { $0.id == drawingID }) {
            ...
            drawingStore.deleteRoute(route)
        } else if let circle = drawingStore.circles.first(where: { $0.id == drawingID }) {
    ...
    private func getDrawingInfo() -> DrawingInfo? {
        if let marker = drawingStore.markers.first(where: { $0.id == drawingID }) {
            ...
            return DrawingInfo(
                ...
                details: "\(route.coordinates.count) points, \(formatDistance(distance))"
            )
        } else if let circle = drawingStore.circles.first(where: { $0.id == drawingID }) {
            return DrawingInfo(
                type: "Circle",
