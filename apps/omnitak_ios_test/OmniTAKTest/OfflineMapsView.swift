...
struct RegionDetailView: View {
    ...
    var body: some View {
        NavigationView {
            List {
                Section("INFORMATION") {
                    DetailRow(label: "Name", value: region.name)
                    DetailRow(label: "Created", value: region.dateCreated.formatted(date: .long, time: .shortened))
                    DetailRow(label: "Size", value: region.formattedSize)
                    DetailRow(label: "Tiles", value: "\(region.downloadedTiles) / \(region.totalTiles)")
                    DetailRow(label: "Progress", value: "\(Int(region.progress * 100))%")
                }

                Section("COVERAGE") {
                    DetailRow(label: "Center", value: String(format: "%.4f, %.4f", region.centerLatitude, region.centerLongitude))
                    DetailRow(label: "Zoom Levels", value: "\(region.minZoom) - \(region.maxZoom)")
                }
