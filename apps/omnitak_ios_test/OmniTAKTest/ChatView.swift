...
struct NewChatView: View {
    ...
    private func selectAllChatUsers() {
        ...
        if let allUsersConv = chatManager.conversations.first(where: { $0.id == ChatRoom.allUsersId }) {
    ...
    private func startDirectChat(with participant: ChatParticipant) {
        let conversation = chatManager.getOrCreateDirectConversation(with: participant)
        dismiss()
