        ...
        VStack(spacing: 12) {
            ...
            VStack(spacing: 8) {
                ...
                DrawingToolButton(
                    ...
                    drawingManager.startDrawing(mode: .route)
                }
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 8) {
                    ...
                    // Routes
                    if !drawingStore.routes.isEmpty {
                        DrawingSection(title: "ROUTES", count: drawingStore.routes.count)
                        ForEach(drawingStore.routes) { route in
                            DrawingListItem(
                                name: route.name,
                                icon: "arrow.triangle.turn.up.right.diamond.fill",
                                color: route.color.swiftUIColor
                            ) {
                                selectedDrawingID = route.id
                                showingProperties = true
                            }
                        }
                    }
